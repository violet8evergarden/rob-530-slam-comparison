...
namespace ORB_SLAM3 {
    ...
    std::string keyNetModelPath() {return keyNetModelPath_;}
    std::string yoloModelPath() {return yoloModelPath_;}
    std::string depthModelType() {return depthModelType_;}
    std::string depthModelPath() {return depthModelPath_;}
    float maxDepthDistance() {return maxDepthDistance_;}
    
    // Get camera intrinsics for depth inference
    void getCameraIntrinsics(float& fx, float& fy, float& cx, float& cy, int& width, int& height) {
        fx = originalCalib1_->getParameter(0);
        fy = originalCalib1_->getParameter(1);
        cx = originalCalib1_->getParameter(2);
        cy = originalCalib1_->getParameter(3);
    ...
    private:
        ...
        void readModelPaths(cv::FileStorage& fSettings);
         ...
         * Model paths
         */
    std::string keyNetModelPath_;
    std::string yoloModelPath_;
    std::string depthModelType_;
    std::string depthModelPath_;
    float maxDepthDistance_;
