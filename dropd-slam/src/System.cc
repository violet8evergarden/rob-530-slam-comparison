    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    // Read dynamic scene configuration
    node = fsSettings["System.DynamicScene"];
    bool bDynamicScene = false;
    if(!node.empty())
    {
        bDynamicScene = static_cast<int>(fsSettings["System.DynamicScene"]) != 0;
    }
    ...
    // Set up the yolo segmentator (only if dynamic scene is enabled)
    if (bDynamicScene)
    {
        string model_path;  
        if (settings_) {
            string yoloPath = settings_->yoloModelPath();
            if (!yoloPath.empty()) {
                model_path = yoloPath;
            }
        } else {
            // Try reading directly from config file
            cv::FileNode node = fsSettings["System.YoloModelPath"];
            if (!node.empty() && node.isString()) {
                model_path = node.string();
            }
        }
        yolo_segmentator = new DropD::yolo::YoloSegmentator(model_path, "yolov11");
    }
    else
    {
        yolo_segmentator = nullptr;
    }
...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageRGBD(imToFeed,imDepthToFeed,timestamp,filename, objs);
...
void System::Shutdown()
{
    ...
    // Cleanup YoloSegmentator if it was initialized
    if (yolo_segmentator != nullptr)
    {
        delete yolo_segmentator;
        yolo_segmentator = nullptr;
    }
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    // Check if we have any keyframes
    if(vpKFs.empty()) {
        cerr << "No keyframes to save!" << endl;
        return;
    }
    ...
    // Safety check: ensure all lists are the same size to prevent iterator corruption
    if(mpTracker->mlpReferences.size() != mpTracker->mlRelativeFramePoses.size() ||
       mpTracker->mlFrameTimes.size() != mpTracker->mlRelativeFramePoses.size() ||
       mpTracker->mlbLost.size() != mpTracker->mlRelativeFramePoses.size()) {
        cerr << "Warning: Tracking lists have different sizes!" << endl;
        cerr << "mlpReferences: " << mpTracker->mlpReferences.size() << endl;
        cerr << "mlFrameTimes: " << mpTracker->mlFrameTimes.size() << endl;
        cerr << "mlbLost: " << mpTracker->mlbLost.size() << endl;
        cerr << "mlRelativeFramePoses: " << mpTracker->mlRelativeFramePoses.size() << endl;
    }
        ...
        lit++, lRit++, lT++, lbL++)
    {
        ...
        // Skip if we couldn't find a valid parent keyframe
        if(!pKF)
            continue;
...
void System::SaveKeyFrameTrajectoryTUM(const string &filename)
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
       ...
       // pKF->SetPose(pKF->GetPose()*Two);
        ...
        // Skip if keyframe is null or bad
        if(!pKF || pKF->isBad())
            continue;
