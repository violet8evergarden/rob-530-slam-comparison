...
namespace ORB_SLAM3 {
    ...
    bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false) {
        ...
        readModelPaths(fSettings);
        cout << "\t-Loaded model paths" << endl;
    ...
    void Settings::readModelPaths(cv::FileStorage& fSettings) {
        bool found;

        keyNetModelPath_ = readParameter<string>(fSettings,"System.KeyNetModelPath",found,false);
        if(!found) {
            keyNetModelPath_ = "";  // Default empty, will use hardcoded fallback
        }

        yoloModelPath_ = readParameter<string>(fSettings,"System.YoloModelPath",found,false);
        if(!found) {
            yoloModelPath_ = "";  // Default empty, will use hardcoded fallback
        }

        depthModelType_ = readParameter<string>(fSettings,"System.DepthModelType",found,false);
        if(!found) {
            depthModelType_ = "";  // Default empty, disabled
        }

        depthModelPath_ = readParameter<string>(fSettings,"System.DepthModelPath",found,false);
        if(!found) {
            depthModelPath_ = "";  // Default empty, disabled
        }

        maxDepthDistance_ = readParameter<float>(fSettings,"System.MaxDepthDistance",found,false);
        if(!found) {
            maxDepthDistance_ = 0.0f;  // Default 0 = no filtering
        }
    }
